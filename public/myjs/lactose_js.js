...
async function submitToFirestore(data) {
    try {
        const materialsCollection = collection(db, "LACTOSE_DB");
        ...
        if (existingDocId) {
            const docRef = doc(db, "LACTOSE_DB", existingDocId);
        ...
        const subCollectionRef = collection(db, `LACTOSE_DB/${existingDocId}/historyAdded`);
        ...
        const formModal = document.getElementById('userFormLactoseList');
    ...
    document.getElementById('databody').addEventListener('click', async function (e) {
        if (e.target.classList.contains('btn-danger')) {
            ...
            if (confirm("คุณต้องการลบข้อมูลนี้ใช่หรือไม่?")) {
                try {
                    await deleteDoc(doc(db, "LACTOSE_DB", docId));
...
async function fetchDataAndDisplay(selectedType, searchText = "") {
    try {
        let q;
        if (selectedType) {
            q = query(collection(db, "LACTOSE_DB"), where("type", "==", selectedType));
        } else if (searchText) {
            q = query(collection(db, "LACTOSE_DB"), where("materialName", "==", searchText));
        } else if (searchText) {
            q = query(collection(db, "LACTOSE_DB"), where("LotNo", "==", searchText));
        } else {
            q = query(collection(db, "LACTOSE_DB"), orderBy("no", "asc"));
        }
        ...
        const fetchPromises = docs.map(async (doc) => { // ใช้ตัวแปร docs แทน querySnapshot.docs
            ...
            const addedDocsPromise = getDocs(collection(db, `LACTOSE_DB/${docId}/historyAdded`));
            const subtractedDocsPromise = getDocs(collection(db, `LACTOSE_DB/${docId}/historySubtracted`));
        ...
        if (existingDocId) {
            ...
            if (docSnap.exists()) {
                ...
                if (newRemaining < 0) {
                    ...
                    const subCollectionRef = collection(db, `LACTOSE_DB/${existingDocId}/historySubtracted`);
                    await addDoc(subCollectionRef, {
                        ...data,
                        remaining: -data.remaining,
                    });
