...
async function loadTypes() {
    ...
    const querySnapshot = await getDocs(collection(db, "typeFlavour"));
...
async function loadFilterOptions() {
    ...
    const typesSnapshot = await getDocs(collection(db, "typeFlavour"));
...
async function submitToFirestore(data) {
    try {
        const materialsCollection = collection(db, "FLAVOUR_DB");
        ...
        if (data.type) {
            const typeRef = collection(db, "typeFlavour");
        ...
        if (existingDocId) {
            const docRef = doc(db, "FLAVOUR_DB", existingDocId);
        ...
        const subCollectionRef = collection(db, `FLAVOUR_DB/${existingDocId}/historyAdded`);
        ...
        const formModal = document.getElementById('userFormFlavourList');
    ...
    document.getElementById('databody').addEventListener('click', async function (e) {
        if (e.target.classList.contains('btn-danger')) {
            ...
            if (confirm("คุณต้องการลบข้อมูลนี้ใช่หรือไม่?")) {
                try {
                    await deleteDoc(doc(db, "FLAVOUR_DB", docId));
...
async function fetchDataAndDisplay(selectedType, searchText = "") {
    try {
        let q;
        if (selectedType) {
            q = query(collection(db, "FLAVOUR_DB"), where("type", "==", selectedType));
        } else if (searchText) {
            q = query(collection(db, "FLAVOUR_DB"), where("materialName", "==", searchText));
        } else if (searchText) {
            q = query(collection(db, "FLAVOUR_DB"), where("LotNo", "==", searchText));
        } else {
            q = query(collection(db, "FLAVOUR_DB"), orderBy("no", "asc"));
        }
        ...
        const fetchPromises = docs.map(async (doc) => { // ใช้ตัวแปร docs แทน querySnapshot.docs
            ...
            const addedDocsPromise = getDocs(collection(db, `FLAVOUR_DB/${docId}/historyAdded`));
            const subtractedDocsPromise = getDocs(collection(db, `FLAVOUR_DB/${docId}/historySubtracted`));
        ...
        if (existingDocId) {
            ...
            if (docSnap.exists()) {
                ...
                if (newRemaining < 0) {
                    ...
                    const subCollectionRef = collection(db, `FLAVOUR_DB/${existingDocId}/historySubtracted`);
                    await addDoc(subCollectionRef, {
                        ...data,
                        remaining: -data.remaining,
                    });
