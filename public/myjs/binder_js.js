...
async function loadTypes() {
    ...
    const querySnapshot = await getDocs(collection(db, "typeBinder"));
...
async function loadFilterOptions() {
    ...
    const typesSnapshot = await getDocs(collection(db, "typeBinder"));
...
async function submitToFirestore(data) {
    try {
        const materialsCollection = collection(db, "BINDER_DB");
        ...
        if (data.type) {
            const typeRef = collection(db, "typeBinder");
        ...
        if (existingDocId) {
            const docRef = doc(db, "BINDER_DB", existingDocId);
        ...
        const subCollectionRef = collection(db, `BINDER_DB/${existingDocId}/historyAdded`);
        ...
        const formModal = document.getElementById('userFormBinderList');
...
document.getElementById('BinderForm').addEventListener('submit', function (e) {
    e.preventDefault();
...
document.getElementById('BinderForm').addEventListener('submit', async function (e) {
    e.preventDefault();
    ...
    document.getElementById('databody').addEventListener('click', async function (e) {
        if (e.target.classList.contains('btn-danger')) {
            ...
            if (confirm("คุณต้องการลบข้อมูลนี้ใช่หรือไม่?")) {
                try {
                    await deleteDoc(doc(db, "BINDER_DB", docId));
...
async function fetchDataAndDisplay(selectedType, searchText = "") {
    try {
        let q;
        if (selectedType) {
            q = query(collection(db, "BINDER_DB"), where("type", "==", selectedType));
        } else if (searchText) {
            q = query(collection(db, "BINDER_DB"), where("materialName", "==", searchText));
        } else if (searchText) {
            q = query(collection(db, "BINDER_DB"), where("LotNo", "==", searchText));
        } else {
            q = query(collection(db, "BINDER_DB"), orderBy("no", "asc"));
        }
        ...
        const fetchPromises = docs.map(async (doc) => { // ใช้ตัวแปร docs แทน querySnapshot.docs
            ...
            const addedDocsPromise = getDocs(collection(db, `BINDER_DB/${docId}/historyAdded`));
            const subtractedDocsPromise = getDocs(collection(db, `BINDER_DB/${docId}/historySubtracted`));
        ...
        if (existingDocId) {
            ...
            if (docSnap.exists()) {
                ...
                if (newRemaining < 0) {
                    ...
                    const subCollectionRef = collection(db, `BINDER_DB/${existingDocId}/historySubtracted`);
                    await addDoc(subCollectionRef, {
                        ...data,
                        remaining: -data.remaining,
                    });
