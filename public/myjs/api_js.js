...
async function submitToFirestore(data) {
    try {
        const materialsCollection = collection(db, "API_LIST");
        ...
        if (existingDocId) {
            const docRef = doc(db, "API_LIST", existingDocId);
        ...
        const subCollectionRef = collection(db, `API_LIST/${existingDocId}/historyAdded`);
        ...
        const formModal = document.getElementById('userFormAPIList');
    ...
    document.getElementById('databody').addEventListener('click', async function (e) {
        if (e.target.classList.contains('btn-danger')) {
            ...
            if (confirm("คุณต้องการลบข้อมูลนี้ใช่หรือไม่?")) {
                try {
                    await deleteDoc(doc(db, "API_LIST", docId));
...
async function fetchDataAndDisplay(selectedType, searchText = "") {
    try {
        let q;
        if (selectedType) {
            q = query(collection(db, "API_LIST"), where("type", "==", selectedType));
        } else if (searchText) {
            q = query(collection(db, "API_LIST"), where("materialName", "==", searchText));
        } else if (searchText) {
            q = query(collection(db, "API_LIST"), where("LotNo", "==", searchText));
        } else {
            q = query(collection(db, "API_LIST"), orderBy("no", "asc"));
        }
        ...
        const fetchPromises = docs.map(async (doc) => {
            ...
            const addedDocsPromise = getDocs(collection(db, `API_LIST/${docId}/historyAdded`));
            const subtractedDocsPromise = getDocs(collection(db, `API_LIST/${docId}/historySubtracted`));
...
${tableBody === databody ? '<td><button class="btn btn-danger">Delete</button></td>' : ''}
`;
        ...
        if (existingDocId) {
            ...
            if (docSnap.exists()) {
                ...
                if (newRemaining < 0) {
                    ...
                    const subCollectionRef = collection(db, `API_LIST/${existingDocId}/historySubtracted`);
                    await addDoc(subCollectionRef, {
                        ...data,
                        remaining: -data.remaining,
                    });
